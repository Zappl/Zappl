ZapplApp.controller("homeCtrl",function($http,$scope,$rootScope, $stateParams, $state, LoginService, $q, $sce, $window,$compile, orderByFilter) {
...
//for createzap word limit remaining
...
//for createzap word limit remaining
  ...
  hc.postActive = function(){
    setTimeout(function(){
      if($window.location.hash =='#!/@'+hc.testCurrentUser){
      ...
      else if($window.location.hash =='#!/@'+hc.testCurrentUser+'/Blog'){
        document.getElementById('postPost').style.color =  '#21a3f2';
        document.getElementById('postBlog').style.color =  'black';
        document.getElementById('postReply').style.color =  '#21a3f2';
      }
      else if($window.location.hash =='#!/@'+hc.testCurrentUser+'/Reply'){
        document.getElementById('postPost').style.color =  '#21a3f2';
        document.getElementById('postBlog').style.color =  '#21a3f2';
        ...
        hc.getUserInfo = function() {
              ...
              if(username == undefined || username == '')
              {
                ...
                rep.then(function(data){
                     ...
                     if($window.location.hash !=='#!/Home' && $window.location.hash !=='#!/Wallet'){
                     ...
                     newObj.forEach(function(obj){
                       ...
                       if(displayPost.match(/\.(jpeg|jpg|gif|png|ico)$/) != null){
                         obj.i = true;
                         obj.v = false;
                         obj.y = false;
                      ...
                      //  console.log("TEXT3 : ", obj.bodyTitle);
      ...
      hc.getUserInfoHome = function(){
                   ...
                   var rep1 = LoginService.getUserProfileImage(obj.author);
                     ...
                     rep2.then(function(data){
                       ...
                       obj.weight = data.data.data.isUserAvailable[0].percent;
                        if(obj.isVote == undefined || obj.isVote == false){
                          obj.isVote = false;
                          //document.getElementById(obj.permlink).src = "images/z11.png";
                        }
                        else{
                          if(obj.weight == 0){
                            obj.isVote = false;
                            ...
                            obj.isVote = true;
                            //document.getElementById(obj.permlink).src = "images/z143.png";
                          }
                        }
                      })
                 })
               ...
               if($window.location.hash =='#!/Home'){
                 ...
                 rep.then(function(data){
                      ...
                      newObj.forEach(function(obj){
                        ...
                        var strng = "/<img[\s]+[^>]*?((alt*?[\s]?=[\s\"\']+(.*?)[\"\']+.*?)|(src*?[\s]?=[\s\"\']+(.*?)[\"\']+.*?))((src*?[\s]?=[\s\"\']+(.*?)[\"\']+.*?>)|(alt*?[\s]?=[\s\"\']+(.*?)[\"\']+.*?>)|>)/g";
                        obj.bodyTitle=obj.bodyTitle.replace(/<a[^>]*>([^<]+)<\/a>/g," ").replace(/[-a-zA-Z0-9@:%_\+.~#?&//=]{2,256}\.[a-z]{2,4}\b(\/[-a-zA-Z0-9@:%_\+.~#?&//=]*)?/g," ").replace(/^<br>|<br>$/g," ").replace(strng," ");
                        //console.log("obj.bodyTitle",obj.bodyTitle);
                          var rep1 = LoginService.getUserProfileImage(obj.author);
                          rep1.then(function(data){
                            obj.profileImage = data.data.data.userProfileImage;
                          })
         ...
         hc.getTagTopicList = function(){
             var rep = LoginService.getTagTopicList('steemit', 30);
             rep.then(function(data){
               hc.TagTopicList = data.data;
             },function(err){
                 console.log('err',err);
                // alert('on error');
             });
...
//this will give full list of tags and topics when click on get more button
...
// this will give follower list of user whose current profile is opened

//PANKHIL WORKING METHOD
...
// chatFollowerListCheckListMethod
...
hc.checkFollower1 = function(){
  ...
  //   alert();
  //   $scope.list1423.push(hc.username);
  //   $scope.list1423.sort();
  //   var text;
  //   var text1;
  //   var data = {};
  ...
  //   var user = {};
  //   user.room = text;
  //   user.displayName = text1;
  //   user.type = 'group';
  //   user.members = $scope.list1423;
  //   user.img = '';
  //   user.chat = '';
  ...
  //   var path = $window.location.origin;
  //   window.location = path+"/#!/Message/"+hc.username+"/group/"+text;
  //   window.location.reload();
  // }
  ...
  //   $scope.users.push(user);
  //   for (var i = 0; i < $scope.list1423.length; i++){
  //     var name = $scope.list1423[i];
  //     data = {
  //       username : name,
  //       room : $scope.users
  //       }
  //     LoginService.setRoomList(data);
  //     }
  // }
...
// chatFollowerListCheckListMethod
        ...
        //    }
         //
        //    var rep =   LoginService.getUserFollowerList(user,startLimit,endLimit);
        //    rep.then(function(data){
        //    $rootScope.getFollowerListByUserName = data.data.data.following_list.userFollowers;
         //
        ...
        //    var list = data.data.data.following_list.userFollowing;
        //    for (var i=0; i< $rootScope.getFollowerListByUserName.length;i++){
        //      var userNameList =  $rootScope.getFollowerListByUserName[i].follower;
         //
        //      var keepGoing = true;
        //      if(list.find(fl =>fl.following == userNameList))
        //      {
        //        keepGoing = false;
        //      }
        //      var test = {
        ...
        // hc.getUserCommentsOnPost = function(){
        //   var rep =   LoginService.getUserCommentsOnPost(hc.postTag,hc.postUser,hc.postPermlink);
        //   rep.then(function(data){
        //     var newObj = data.data.data.commentsOnPost.comments;
        //     newObj.forEach(function(obj){
        //       var rep1 = LoginService.getUserProfileImage(obj.author);
        //       rep1.then(function(data){
        //         obj.profileImage = data.data.data.userProfileImage;
        ...
        //   };
         ...
         hc.getUserPostContent = function(){
           ...
           //  if(text.match(/(<iframe.+?<\/iframe>)/g)){
          //    text = text.replace(/(<iframe.+?<\/iframe>)/g,'');
          //  }
           if(text.match(/<(button|a)(?!([^>]*?)analytics-on)(?:[^>]*?)>(?:[^<]*)<\/(button|a)>/g)){
             text = text.replace(/<(button|a)(?!([^>]*?)analytics-on)(?:[^>]*?)>(?:[^<]*)<\/(button|a)>/g,'');
           }
...
hc.postVote123 = function(author,permlink,vote1){
      ...
      hc.postCommentOnComment = function(currentUser,tags,title){
        //var test = document.getElementById("comments").value;
    ...
    //              var value = '<img src="'+imageUrl+data.key+' ">';
...
// hc.youTube = function(){
//   var url = document.getElementById("youtubeVideo").value;
//   var body = document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML;
...
//       }
//
//   var test = video.replace(/(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<iframe width="420" height="345" src="http://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>');
//
// console.log("test",test);
//
//   if(test.slice(0,8) == 'https://'){
//     document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML = body+test.substring(8);
//   }
//   else{
//     document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML = body+test;
//   }
...
// hc.youTube = function(){
//   var video = document.getElementById("youtubeVideo").value;
//   var body = document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML;
//   // var testVideo = '<iframe width="420" height="345" src="'+video+'" frameborder="0" allowfullscreen></iframe>';
//   var test = video.replace(/(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<iframe width="420" height="345" src="http://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>');
//   if(test.slice(0,8) == 'https://'){
//     document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML = body+test.substring(8);
//   }
   ...
   s3.upload({
     ...
     else
     {
       console.log("data",data);
         ...
         var value;
         if(file.type.slice(0,5) == 'image'){
          //value = '<img src="'+imageUrl+data.key+'" class="img-responsive">';
          ...
          finalPostKV.push(list);
            ...
            // value = '<video width="500" height="500" controls><source src="'+imageUrl+key+'" type="'+file.type+'"></video>';
...
hc.deleteDraft = function(){
        if(document.getElementById("idField").value !== ''){
          var draftPost = {
                draftId : document.getElementById("idField").value
            }
      ...
      //     var draftData = {userName:userName,draftId:draftId,title:title,body:body,upvote:upvote,rewards:rewards}
      ...
      //   console.log(document.getElementById("idField").value);
      //   if(hc.data.upvote == 1){
      ...
      //   document.getElementById("noiseWidgIframe").contentWindow.document.body.innerHTML = hc.data.body;
      //   switch(hc.data.rewards){
      //     case 100:
      //     document.getElementById("sel1").value  =  "Power Up 100%"
      //     break;
      //     case 50:
      //     document.getElementById("sel1").value  =  "Default (50% / 50%)"
      //     break;
      //     case 0:
      //     document.getElementById("sel1").value  =  "Decline Payout"
      //     break;
      //     case 1:
      ...
      hc.postDeleteComment = function(permlink){
        ...
        setTimeout(function(){ hc.getUserInfoHome(); }, 2000);
      };
...
//
//
// $scope.source_string = 'The details that were given to you for the vps should work i just logged in just fine with the password i gave you.You were provided with the ssh credentials for the server the same time you were given the ios and android credetials';
