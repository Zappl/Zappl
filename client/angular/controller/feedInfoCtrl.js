...
fi.username=localStorage.getItem("username");fi.postingWif=localStorage.getItem("postingWif");fi.getUserFeed=function(startLimit,endLimit){var rep=LoginService.getUserFeed(fi.username,startLimit,endLimit);rep.then(function(data){var newObj=data.data.data.userFeed.userPostInfo;newObj.forEach(function(obj){var displayPost=obj.image;if(displayPost==null){displayPost="";}
...
$rootScope.userFeed=newObj;},function(err){console.log("err",err);});};fi.getFeedTagInfo=function(startLimit,endLimit){if(fi.tag==''||fi.tag==undefined){var rep=LoginService.getFeedInfo(fi.feedType,startLimit,endLimit);}
else{var rep=LoginService.getFeedTagInfo(fi.feedType,fi.tag,startLimit,endLimit);}
