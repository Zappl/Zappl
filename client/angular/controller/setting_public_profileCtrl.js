...
else{$scope.wrongWif="Please Enter Correct Private Active Key";}}};sc.getUserInfo=function(){if(sc.loginToken!=null){var rep=LoginService.getUserInfo(sc.username);rep.then(function(data){sc.userInfo=data.data.data.userInfo;sc.userFInfo=data.data.data.userFollowingFollowersInfo;var profileImage=localStorage.getItem("profileImage");var image;if(profileImage==''||profileImage==null||profileImage==undefined){image=sc.userInfo.profile_image;}
