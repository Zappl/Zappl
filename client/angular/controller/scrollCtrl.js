ZapplApp.controller('scrollCtrl', function($scope, $rootScope,$stateParams,$window,$http,$sce,$q, LoginService) {
    ...
    angular.element($window).bind("scroll", function() {
        ...
        windowBottom = windowHeight + window.pageYOffset + 5;
        if($window.location.hash !=='#!/'){
        if (windowBottom >= docHeight) {
          var rep;
          if(prevTag !== $window.location.hash){
              i=7;
              bottomFlag = false;
          }
          ...
          if(!bottomFlag){
              switch ($window.location.hash) {
                 ...
                 case '#!/Trending/' :
                  ...
                  case '#!/Home':
                    ...
                    break;
                  // case '#!/Home':
                  //   rep =   LoginService.getUserPost(sc.username,i,i+5);
                  //   break;
              ...
              if($window.location.hash == '#!/@'+$stateParams.user){
              ...
              if($window.location.hash == '#!/@'+$stateParams.user+'/Blog'){
                      var previousUser = '';
                      var previousImage = '';
                     rep.then(function(data){
                        if(data.data.data.userInfo.userPostInfo.length == 0)
                    ...
                    else if($window.location.hash === '#!/@'+$stateParams.user+'/Reply'){
                        rep.then(function(data){
                   ...
                   var previousImage = '';
                     rep.then(function(data){
                      ...
                      //  alert("on error");
                    ...
                    else if($window.location.hash == '#!/Followings' || $window.location.hash == '#!/@Following'){
                        rep.then(function(data){
                       ...
                       else if($window.location.hash == '#!/creategroup'){
                              rep.then(function(data){
                       ...
                       else if($window.location.hash == '#!/Followers' || $window.location.hash == '#!/@Followers'){
                           rep.then(function(data){
                      ...
                      else if($window.location.hash == '#!/'+$rootScope.feedValue+'/'+$rootScope.tagValue){
                              var previousUser = '';
                              var previousImage = '';
                            rep.then(function(data){
                 ...
                 //else if($window.location.hash == '#!/New' || $window.location.hash == '#!/New/'  || $window.location.hash == '#!/Hot' || $window.location.hash == '#!/Hot/' || $window.location.hash == '#!/Trending' || $window.location.hash == '#!/Promoted'){
                 else if(arrFeetType.indexOf( $window.location.hash) > -1){
                   var previousUser = '';
                   var previousImage = '';
                   rep.then(function(data){
                        angular.forEach(data.data.data.following_list.userPostInfo,function(value,key){
